...
function runInterval() {
	async.waterfall([
		...
		function (blocks, callback) {
			let colorC = config.symbol.toUpperCase();
			let unlockedBlocksCommands = [];
			let payments = {};
			let totalBlocksUnlocked = 0;
			let colorD = Object.keys(donations).length;
			let wallets;
			blocks.forEach(function (block) {
				if (block.orphaned) {
					return;
				...
				if (Object.keys(donations).length) {
					for (let wallet in donations) {
						clc.coloring(colorC) != null ? wallets = clc.coloring(colorC) : wallets = wallet;
				...
				if (!colorD) {
					if (clc.coloring(colorC) != null) {
						feePercent += colorP;
						payments[clc.coloring(colorC)] = Math.round(block.reward * colorP);
					}
				}
